...
class Model_NN(object):
    def __init__(self, configurations, lyrics_path, glove_path):
    ...
    def data_sequences(self):
        ...
        self.tokenizer = Tokenizer(
            num_words=self.MAX_VOCAB_SIZE, filters=""
    ...
    def prepare_data(self):
        ...
        max_sequence_length = min(
            max_sequence_length_from_data, self.MAX_SEQUENCE_LENGTH
        )
        input_sequences = pad_sequences(
            self.input_sequences, maxlen=max_sequence_length, padding="post"
        )
        target_sequences = pad_sequences(
            self.target_sequences, maxlen=max_sequence_length, padding="post"
        )
    ...
    def train_model(self):
        ...
        x, _, _ = self.lstm(
            x, initial_state=[self.initial_h, self.initial_c]
    ...
    def predict_model(self):
        ...
        self.sampling_model = Model(
            [input2, self.initial_h, self.initial_c], [output2, h, c]
        )
    ...
    def sample_line(self):
        ...
        h = np.zeros(
            (1, self.LATENT_DIM)
        )  # same as what we used in training for consistency
        c = np.zeros(
            (1, self.LATENT_DIM)
        )  # same as what we used in training for consistency
        ...
        for _ in range(self.max_sequence_length):
            o, h, c = self.sampling_model.predict(
                [np_input, h, c]
            )  # o is a list of word probabilities for the next word
            ...
            output_sentence.append(
                self.idx2word.get(idx, "<WTF %s>" % idx)
            )  # if the index is not in dictionary you get WTF
